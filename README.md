...
`qimo-speedrun` is an Agent Skill for university finals review. It turns messy course materials into a 10-12 hour, source-grounded sprint path: material coverage, teacher-priority extraction, knowledge framework, guided original courseware review, courseware question bank, staged mock exams, active coaching, mistake review, rolling recitation, scheduled follow-ups, and final memory sheet.
...
- Guided original courseware review: after the framework, after Exam 1, and before final sprint, the student audits the original PPT/PDF/handout instead of blindly trusting generated summaries.
- Active coaching feedback: the agent infers memorization difficulty, unclear framework, fatigue, low initiative, and pace risk, then proposes concrete next moves.
...
- Ordinary web AI without skill installation, file access, and persistent state is not a supported target. Hermes message gateways are supported because they remain part of the installed agent workflow.
...
3. Use message gateways through Hermes or another installed agent when you want QQ/WeChat reminders.
...
```text
Use $qimo-speedrun. I have one unsorted course folder for a final exam. First build source coverage and exam rules; include guided original courseware review after the framework; if scheduled tasks/message reminders are available, ask me before creating timed check-ins.
```
